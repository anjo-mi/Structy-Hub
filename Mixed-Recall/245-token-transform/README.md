...
Write a function, _tokenTransform_, that takes in an object of tokens and a string. In the object,
the replacement values for a token may reference other tokens. The function should return a new
string where tokens are replaced with their fully evaluated string values.

Tokens are enclosed in a pair of
'$'. You can assume that the input string is properly formatted and '$' is not used in the string
except to enclose a token.
